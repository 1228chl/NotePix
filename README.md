...
- **基于标题层级的绝对编号命名**：  
  图片文件名格式为 `层级路径-计数器.扩展名`（例如 `1.1.3-1.png`）。  
  插件会基于整个文档的标题树生成绝对编号（类似 Word 多级列表），而非简单的行扫描。计数器会跨会话持久化。  
  您可以在设置中调整 **最大标题深度**（1~6 级），超出部分将被截断。

- **公开图片链接格式切换（Raw ↔ CDN）**：  
  支持 `GitHub Raw` 和 `jsDelivr CDN` 两种格式。CDN 在国内访问更快（但有 24 小时缓存），Raw 链接无缓存但速度较慢。  
  可在设置中全局选择默认格式，也支持 **右键单张转换** 或 **批量转换整个笔记**。

- **重新整理图片序号**：  
  提供命令 `NotePix: 重新整理当前笔记的图片序号`。  
  插件会自动扫描当前笔记中每个标题层级下的图片，将其序号重新编排为连续数字（从 1 开始），并处理重命名、重新上传及链接更新，确保无空缺。
...
2. 如果仓库是公开的 → 根据设置选择 `GitHub Raw` 或 `jsDelivr CDN` 链接。
3. 如果仓库是私有的 → 插入私有内部链接（v2 格式），例如 `![](obsidian://notepix/v2/<所有者>/<仓库>/<分支>/<路径>)`。

#### 公开模式
- 上传总是插入公开链接（格式由“公开图片链接格式”决定）。
...
| **公开图片链接格式** | 仅当仓库为公开时生效。`GitHub Raw` 无缓存但速度较慢；`jsDelivr CDN` 国内访问更快但有 24 小时缓存。 | `GitHub Raw` / `jsDelivr CDN` |
| **标题层级最大深度** | 生成文件名时最多使用几级标题序号（1-6）。超出部分将被截断，避免文件名过长。 | `3`（默认） |
...
| **删除前确认** | 删除 GitHub 上的图片前显示确认对话框。 | 开启/关闭 |
...
- **批量转换链接格式**：在设置面板中，选择目标格式后点击“转换当前笔记链接”，即可将当前笔记中的所有图片链接在 Raw 和 CDN 之间批量转换。
- **右键单张转换**：在编辑器或阅读视图中，右键点击图片，选择“转换图片链接为 xxx”即可快速切换链接格式。
- **重新整理图片序号**：运行命令面板中的 `NotePix: 重新整理当前笔记的图片序号`，插件将自动检测空缺并重新上传，保持序号连续。
